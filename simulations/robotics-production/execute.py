#!/usr/bin/env python3
"""Reproduce the flagship robotics-production simulation with isolated local APEX state."""
...
RUN_ROOT=Path('/tmp/axiom-robotics-production-sr1-001')
...
def main(argv=None):
    ...
    apex_url=f'http://127.0.0.1:{port}'; key='robotics-simulation-local-key'
    with tempfile.TemporaryDirectory(prefix='axiom-robotics-apex-') as td:
        ...
        try:
            ...
            evidence,evaluation=runner.run(org,scenario,fixture,request,RUN_ROOT/'terminal-state.json',apex_url=apex_url,authority_ref='robotics-release-board:synthetic-approved',api_key=key)
