...
reproducible execution in consequential environments. We present AXIOM
APEX (Autonomous Plan EXecutor), a plan-deterministic swarm runtime that
enforces schema-validated, bounded, process-isolated execution contracts
over LLM-generated plans, governed by a native pre-execution policy
enforcement layer (the AXIOM APEX Validator) that validates and sanitizes
plan submission prior to execution. Together, these systems instantiate a
...
\item
  ...
  trace.
\item
  A native Policy Validation Layer implementing blast-radius
...
orchestration frameworks lack the pre-execution enforcement layer that
distinguishes our native validator from prior work. LangGraph provides
a stateful execution graph but no policy layer: tool invocation is
unrestricted by blast-radius or allowlist constraints. Temporal enforces
workflow durability and retry semantics but has no LLM-native contract
mechanism --- policies are defined in application code, not validated
against plan structure. AutoGen supports multi-agent coordination but
provides no execution bounds on individual agent actions. Our native
validator's distinguishing contribution is enforcement at the API
boundary: blast-radius classification, tool allowlisting, and step-count
limits are validated against the plan structure before any execution begins.
...
Safety
Primitive}\label{3-theoretical-framework-information-flow-as-the-core-safety-primitive}

\subsubsection{3.1 The Type-Confusion Model of Prompt
Injection}\label{31-the-type-confusion-model-of-prompt-injection}
...
Envelope}\label{32-verification-as-a-correctness-envelope}
...
Constraints}\label{33-recursive-self-improvement-and-stability-constraints}
...
Runtime}\label{4-apex-deterministic-agent-runtime}

\subsubsection{4.1 Core Execution
Contract}\label{41-core-execution-contract}
...
\subsubsection{4.2 Execution Bounds}\label{42-execution-bounds}
...
Autoloading}\label{43-tool-registry-and-autoloading}
...
\subsubsection{4.4 Audit Infrastructure}\label{44-audit-infrastructure}

Every run emits a JSONL trace recording each step\textquotesingle s tool
invocation, arguments, result, and wall time. Runs are persisted to a
SQLite database (\texttt{\textasciitilde{}/.apex/runs.db}) with schema:
...
Abstraction}\label{45-multi-provider-abstraction}
...
\subsubsection{4.6 HTTP API}\label{46-http-api}
...
\subsection{5. Native Policy Enforcement Layer}\label{5-native-policy-enforcement-layer}

\subsubsection{5.1 Architecture}\label{51-architecture}

The AXIOM APEX Validator is a pre-execution policy enforcement layer that
validates and governs plan submission to the executor. It implements a strict
separation: the core loop defines execution semantics, while the validator
defines submission policy. Plans that violate policy are rejected before
reaching the execution loop.

\subsubsection{5.2 Policy Schema}\label{52-policy-schema}

System policies are Pydantic-validated contracts:
...
The \texttt{blast\_radius} field classifies the
execution\textquotesingle s potential impact envelope. \texttt{none}
permits read-only operations; \texttt{local} permits filesystem
modification within the process working directory; \texttt{network}
permits egress. Policy is enforced at the structural level prior to
execution, not as a runtime advisory.

\subsubsection{5.3 Validation Logic}\label{53-validation-logic}
...
\tightlist
\item
  \texttt{len(plan.steps)\ \le\ policy.max\_steps}
\item
  No step invokes a tool in the blocked set (\texttt{\{"shell"\}} by
  default)
\item
  If \texttt{policy.allowed\_tools} is non-empty, every
  step\textquotesingle s tool must appear in it
\item
...
\subsubsection{5.4 Rollback Generation}\label{54-rollback-generation}

On execution failure, the system generates a compensating rollback plan
...
(BDSO)}\label{6-benchmark-driven-self-optimization-bdso}

\subsubsection{6.1 Bounded BDSO Loop}\label{61-bounded-bdso-loop}

APEX implements a Benchmark-Driven Self-Optimization loop
(\texttt{apex\ rsi}) that generates candidate patches to the
executor\textquotesingle s own source files, scores them via the
benchmark harness, and applies improvements that exceed a fitness
threshold.
...
\subsubsection{6.2 Fitness Function}\label{62-fitness-function}
...
\tightlist
\item
  \texttt{pass\_rate\ =\ passed\_tasks\ /\ total\_tasks}
\item
...
\subsubsection{6.3 Security Boundary}\label{63-security-boundary}
...
\subsection{7. Evaluation}\label{7-evaluation}

\subsubsection{7.1 Benchmark Harness}\label{71-benchmark-harness}
...
\subsubsection{7.2 Task Suite}\label{72-task-suite}
...
\subsubsection{7.3 Results}\label{73-results}
...
\toprule\noalign{}
Run & Pass Rate & Wall Time & \texttt{speed\_factor} &
\texttt{token\_efficiency} & \texttt{apex\_score} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\subsection{8. Docker Deployment}\label{8-docker-deployment}

APEX is containerized and orchestrated via docker-compose. The compose
stack defines the primary execution container, which builds directly from the
source, binds port 8080, and exposes an automated healthcheck on GET /health
while persisting configuration states, database records, and run logs to a
named volume. This allows production deployment of the full plan-deterministic
architecture as a single docker-compose up command with credentials securely
injected at runtime.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{9. Future Work}\label{9-future-work}
...
would enable domain-specific policy without manual specification.
...
a bounded executor acts. AXIOM APEX instantiates this architecture
concretely, combining a deterministic execution loop with a native
pre-execution policy validator, complete audit infrastructure, and a
BDSO loop bounded by a composite fitness function and a permanent
...
\texttt{github.com/axiom-llc/axiom-apex}.
...
Greshake, K., et al. (2023). Not What You\textquotesingle ve Signed Up
For: Compromising Real-World LLM-Integrated Applications with Indirect
Prompt Injection. \emph{arXiv:2302.12173}.
...
\item
  Pre-execution policy enforcement is always active
\item
...
subset)}\label{appendix-b-benchmark-task-suite-tasksjson-nominal-subset}
