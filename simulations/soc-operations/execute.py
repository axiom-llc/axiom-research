...
RUN_ROOT=Path('/tmp/axiom-soc-operations-001')
...
def main(argv=None):
    ...
    apex_url=f'http://127.0.0.1:{port}'; key='soc-operations-simulation-local-key'
    with tempfile.TemporaryDirectory(prefix='axiom-robotics-apex-') as td:
        ...
        try:
            ...
            evidence,evaluation=runner.run(org,scenario,fixture,request,RUN_ROOT/'terminal-state.json',apex_url=apex_url,authority_ref='incident-commander:synthetic-approved',api_key=key)
